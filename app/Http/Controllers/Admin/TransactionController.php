...
class TransactionController extends Controller
{
    public static function idPay(Request $request, $price, $product_id = 0)
    {
        ...
        $transaction->update(['transaction_data' => ['data' => ['token' => Crypt::encryptString($result->id), 'price' => Crypt::encryptString($price)]]]);
        return $result;
    }
    ...
    public function idPayCallback(Request $request)
    {
        ...
        if ($request->status != 10) {
            $transaction->forceFill(['transaction_data->data->info' => Crypt::encryptString('Not paid -- '), 'transaction_data->data->result' => $requesteq]);
        ...
        if (isset($transaction->transaction_data['data']['track_id'])) {
            $transaction->forceFill(['transaction_data->data->info' => Crypt::encryptString('Repeated request -- '), 'transaction_data->data->result' => $requesteq]);
        ...
        if (Crypt::decryptString($transaction->transaction_data['data']['token']) != $request->id) {
            $transaction->forceFill(['transaction_data->data->info' => Crypt::encryptString('Different ID from IDPay -- '), 'transaction_data->data->result' => $requesteq]);
        ...
        $transaction->forceFill(['transaction_data->data->track_id' => Crypt::encryptString($request->track_id), 'transaction_data->data->result' => $requesteq]);
        ...
        if (!isset($result->status) || $result->status != (100 || 200)) {
            $transaction->forceFill(['transaction_data->data->info' => Crypt::encryptString('Could not verify'), 'transaction_data->data->result' => $requesteq]);
            ...
            $transaction->forceFill(['transaction_data->data->info' => $result, 'transaction_data->data->paid' => true]);
